...
(script_directory / "orthos" / "orthos2dev.env").write_text(
    ...
    'ALLOWED_HOSTS="orthos2 orthos2.orthos2.test"\n'
...
# production entrypoint (docker/orthos/production-server.sh) sees the same secrets as the
# devel entrypoint. NetboxToken can't be generated here: it's provisioned by calling
# a running Netbox instance's API (see get_netbox_token() in docker/orthos/devel-server.sh).
# `make up-testing` handles this automatically via docker/orthos/get-netbox-token.sh.
