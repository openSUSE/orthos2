...
@register.simple_tag
def url_replace(request: HttpRequest, field: str, value: Any, remove: bool = False):
    """
...
@register.simple_tag
def status_ipv4(machine: Machine):
...
@register.simple_tag
def status_ipv6(machine: Machine):
    if machine.check_connectivity < Machine.Connectivity.PING:
        return mark_safe(
            '<td class="blue"><span class="text-small">Disabled</span></td>'
        )
...
@register.simple_tag
def status_ssh(machine: Machine):
...
@register.simple_tag
def status_login(machine: Machine):
...
@register.simple_tag
def order_list(request: HttpRequest, field: str):
...
@register.simple_tag
def vm_record(request: HttpRequest, vm: Machine):
