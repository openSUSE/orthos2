...
@register.simple_tag
def status_ssh(machine):
    if machine.check_connectivity < Machine.Connectivity.SSH:
        return mark_safe('<td class="blue"><span class="text-small">Disabled</span></td>')

    if machine.status_ssh:
        result = '<td class="green" title="SSH port open"><i class="fa fa-check"></i></td>'
    else:
        result = '<td class="red" title="SSH port not open"><i class="fa fa-close"></i></td>'

    return mark_safe(result)


@register.simple_tag
def status_login(machine):
...
def order_list(request, field):
    ...
    return mark_safe('<a href="{}">{}</a><a href="{}">{}</a>'.format(url_asc, up, url_desc, down))
