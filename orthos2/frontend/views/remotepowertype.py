...
from orthos2.data.models import RemotePowerDevice, RemotePowerType
...
class RemotePowerTypeListView(SuperuserRequiredMixin, ListView):
    ...
    def get_queryset(self) -> QuerySet["RemotePowerType"]:
        return super().get_queryset().annotate(device_count=Count("remotepowerdevice"))  # type: ignore
...
def remotepowertype_devices(request: HttpRequest, id: int) -> HttpResponse:
    ...
    return render(
        ...
        {
            "remotepowertype": remotepowertype,
            "remotepowerdevices": RemotePowerDevice.objects.filter(
