...
"""
This script will run on Orthos 2 Production with Git tag 1.5 and verify that all data that Orthos 2 needs in version 1.6
is present and can be relied upon.
...
class REST:
    ...
    def uploader(self, data, url: str):
        method = "POST"
...
class NetBoxCheck(REST):
    ...
    def check_ip_by_vm_interface(self, id):
...
def main() -> None:
    ...
    for intf in NetworkInterface.objects.all():
        orthos_interface_mac = intf.mac_address
        orthos_machine_id = intf.machine.id
        orthos_is_vm = intf.machine.system.virtual
        orthos_is_diskarray = intf.machine.system.name == "DiskArray"
        logger.debug(
        ...
        netbox_device_interfaces = netbox_api.check_mac_address(orthos_interface_mac)
    ...
    for bmc in BMC.objects.all():
        orthos_bmc_fqdn = bmc.fqdn
        orthos_bmc_mac = bmc.mac
        orthos_machine_id = bmc.machine.id
