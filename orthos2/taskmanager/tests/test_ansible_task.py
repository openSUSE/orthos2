...
class AnsibleTaskTest(TestCase):
    ...
    def setUp(self) -> None:
        ...
        # Re-enable logging in case other tests disabled it
    ...
    def test_execute_logs_missing_machines(
        ...
        with self.assertLogs("tasks", level="WARN") as cm:
            task = Ansible(machines)
            task.execute()
        ...
        assert (
            "No scan results found in database for machines: missing.orthos2.test"
            in log_output
        )
