...
class CobblerServer:
    ...
    def add_machine(
        ...
        default_profile_uid = self._get_profile_uid(default_profile)
        ...
        if save == CobblerSaveModes.NEW:
            ...
            old_machine_has_bmc = self._xmlrpc_server.has_item(  # type: ignore
            ...
            # Cobbler >=4.0.0 nests power fields under a "power" sub-object (see
            # cobbler/items/options/power.py) rather than flat "power_type" etc. keys.
            if old_machine_dict.get("power", {}).get("type", "") != "":
        ...
        self._xmlrpc_server.modify_system(
            object_id, ["profile"], default_profile_uid, self._token
        )
        ...
        if tftp_server:
            # Cobbler >=4.0.0 nests these under a "tftp" sub-object (see
            # cobbler/items/options/tftp.py) rather than flat "next_server_v4"/
            # "next_server_v6" properties on the system itself.
    ...
    def add_power_options(
        ...
        self._xmlrpc_server.modify_system(
            object_id, ["power", "type"], fence.name, self._token
        )

        if fence.identity_file == "":
            username, password = remotepower.get_credentials()
            self._xmlrpc_server.modify_system(
                object_id, ["power", "user"], username, self._token
            )
            self._xmlrpc_server.modify_system(
                object_id, ["power", "password"], password, self._token
            )
        else:
            self._xmlrpc_server.modify_system(
                object_id, ["power", "user"], fence.username, self._token
            )
            self._xmlrpc_server.modify_system(
                object_id,
                ["power", "identity_file"],
        ...
        if fence.use_hostname_as_port:
            ...
            self._xmlrpc_server.modify_system(
                object_id, ["power", "id"], get_hostname(machine.hostname), self._token  # type: ignore
            )
        elif fence.use_port:
            ...
            if fence.name == "raritan":
                ...
                self._xmlrpc_server.modify_system(
                    object_id, ["power", "id"], remotepower.port, self._token
                )

        self._xmlrpc_server.modify_system(
            object_id,
            ["power", "address"],
            remotepower.get_power_address(),
            self._token,
        )
        if remotepower.options != "":
            self._xmlrpc_server.modify_system(
                object_id, ["power", "options"], remotepower.options, self._token
            )
    ...
    def add_remote_power_device(
        ...
        """
        default_profile = device.architecture.default_profile
        if not default_profile or not self._xmlrpc_server.has_item(
            "profile", default_profile, self._token
        ):
            raise CobblerException(
                f"profile {default_profile} didn't exist on cobbler server"
            )
        ...
        self._xmlrpc_server.modify_system(
            object_id,
            ["profile"],
            self._get_profile_uid(default_profile),
            self._token,
        )
    ...
    def _get_cobbler_datastructure(self, machine: "Machine") -> Dict[str, Any]:
        ...
        system_dict = self._xmlrpc_server.get_system(
            system_handle, False, False, self._token
        )
    ...
    @login_required
    def _get_profile_uid(self, profile_name: str) -> str:
        """
        Resolve a Cobbler profile name to its uid.

        System.profile (see cobbler/items/system.py) is set by uid, not by name, unlike
        e.g. has_item()/find_profile() which operate on the name. get_profile_handle()
        (like get_item_handle()/get_<type>_handle() in general) remains name-based under
        Cobbler >=4.0.0's uid-based get_item()/get_<type>() contract, and already resolves
        straight to the item's uid, so no separate get_profile() call is needed here.

        :param profile_name: Name of the profile to resolve.
        """
        uid = self._xmlrpc_server.get_profile_handle(profile_name)
        if not isinstance(uid, str):
            raise TypeError(
                "Cobbler Server returned incorrect data type for profile uid"
            )
        return uid
    ...
    def remove(self, machine: "Machine") -> None:
        ...
        try:
            # remove_system() requires an item handle (uid), not the name, unlike
            # get_item_handle()/get_<type>_handle() which remain name-based - resolve it
            # first. If the system no longer exists, get_system_handle() doesn't raise;
            # it returns an unresolvable handle, and remove_system() below then just
            # returns False rather than raising, matching the previous name-based
            # behavior for a since-deleted/unknown system.
            system_handle = self._xmlrpc_server.get_system_handle(machine.fqdn)
            # recursive=True: Cobbler >=4.0.0 stores a system's NetworkInterfaces
            # as separate items, not embedded sub-objects - a non-recursive
            # removal is refused with "removal would orphan network_interface(s)"
            # for any system that has one.
            self._xmlrpc_server.remove_system(system_handle, self._token, True)
    ...
    def remove_by_name(self, fqdn: str) -> None:
        ...
        try:
            # See remove()'s comment above: resolve name -> uid via get_system_handle()
            # first, and recursive=True for the same reason.
            system_handle = self._xmlrpc_server.get_system_handle(fqdn)
            self._xmlrpc_server.remove_system(system_handle, self._token, True)
        except xmlrpc.client.Fault as xmlrpc_fault:
            logging.error(
    ...
    def remove_bmc(
        ...
        if self._xmlrpc_server.has_item(
            ...
            # resolve it first, same as add_bmc()/add_network_interfaces() already do
            # for modify_network_interface()/save_network_interface().
            bmc_handle = self._xmlrpc_server.get_network_interface_handle(bmc_intf_name)
            self._xmlrpc_server.remove_network_interface(bmc_handle, self._token)
    ...
    def remove_power_options(
        ...
        :param save: Whether to save the machine or not.
        """
        self._xmlrpc_server.modify_system(object_id, ["power", "type"], "", self._token)
        self._xmlrpc_server.modify_system(object_id, ["power", "user"], "", self._token)
        self._xmlrpc_server.modify_system(
            object_id, ["power", "identity_file"], "", self._token
        )
        self._xmlrpc_server.modify_system(
            object_id, ["power", "password"], "", self._token
        )
        self._xmlrpc_server.modify_system(object_id, ["power", "id"], "", self._token)
        self._xmlrpc_server.modify_system(
            object_id, ["power", "address"], "", self._token
        )
        self._xmlrpc_server.modify_system(
            object_id, ["power", "options"], "", self._token
        )
        if save != CobblerSaveModes.SKIP:
            self._xmlrpc_server.save_system(
                object_id, True, True, save.value, self._token
    ...
    def setup(self, machine: "Machine", choice: str) -> None:
        ...
        object_id = self._xmlrpc_server.get_system_handle(machine.fqdn)
        if choice:
            profile_name = f"{machine.architecture}:{choice}"
            self._xmlrpc_server.modify_system(
                object_id,
                ["profile"],
                self._get_profile_uid(profile_name),
                self._token,
            )
