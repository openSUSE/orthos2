...
class CobblerServer:
    ...
    def _get_profile_uid(self, profile_name: str) -> str:
        ...
        e.g. has_item()/find_profile() which operate on the name.

        :param profile_name: Name of the profile to resolve.
        """
        profile_dict = self._xmlrpc_server.get_profile(
            profile_name, False, False, self._token
        )
        if not isinstance(profile_dict, dict):
            raise ValueError(
                "Cobbler Server didn't return a dictionary for profile %s"
                % profile_name
            )
        uid = profile_dict.get("uid")
    ...
    def remove(self, machine: "Machine") -> None:
        ...
        try:
            ...
            self._xmlrpc_server.remove_system(machine.fqdn, self._token, True)
    ...
    def remove_by_name(self, fqdn: str) -> None:
        ...
        try:
            # recursive=True: see remove()'s comment above.
            self._xmlrpc_server.remove_system(fqdn, self._token, True)
        except xmlrpc.client.Fault as xmlrpc_fault:
            logging.error(
