...
class CobblerServer:
    ...
    def deploy(self, machines: Iterable["Machine"]):
        ...
        """
        server = CobblerServer(self._domain)
        for machine in machines:
            try:
                server.update_or_add(machine)
    ...
    def add_serial_console(
        ...
        if console.kernel_device != "None":
            # FIXME: Get kernel options and combine them correctly
            kernel_options = {
                "console": f"{console.kernel_device}{console.kernel_device_num},{console.baud_rate}"
            }
            self._xmlrpc_server.modify_system(
                system_handle, "kernel_options", kernel_options, self._token
            )
        if save != CobblerSaveModes.SKIP:
            self._xmlrpc_server.save_system(system_handle, self._token, save.value)
    ...
    def get_profiles(self, architecture: str) -> List[str]:
        ...
        """
        return self._xmlrpc_server.find_profiles(
