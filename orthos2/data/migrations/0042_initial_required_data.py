...
def create_initial_serverconfigs(app_registry: Apps, schema_editor: BaseDatabaseSchemaEditor):
    ServerConfig = app_registry.get_model("data", "ServerConfig")
    db_alias = schema_editor.connection.alias
    ServerConfig.objects.using(db_alias).bulk_create(
        [
            ServerConfig(key="orthos.bugreport.url", value="https://github.com/openSUSE/orthos2/issues"),
            ServerConfig(key="orthos.cli.url", value="https://build.opensuse.org/package/show/systemsmanagement:orthos2:production/orthos-client"),
            ServerConfig(key="orthos.documentation.url", value="https://orthos2.rtfd.io/"),
            ServerConfig(key="orthos.api.welcomemessage", value="Come in, reserve and play...")
        ],
        ignore_conflicts=True
    )


def create_initial_architectures(app_registry: Apps, schema_editor: BaseDatabaseSchemaEditor):
